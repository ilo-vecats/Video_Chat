...
def handle_join(data):
    ...
    # Validate room name
    if not room_id:
        emit('room_error', {'message': 'Room name cannot be empty'}, room=sid)
        return
    
    # Sanitize room name: only allow alphanumeric, hyphens, and underscores, max 50 chars
    room_id = re.sub(r'[^a-zA-Z0-9_-]', '', room_id)
    if len(room_id) > 50:
        room_id = room_id[:50]
    
    if not room_id:
        emit('room_error', {'message': 'Invalid room name. Use only letters, numbers, hyphens, and underscores.'}, room=sid)
        return
    
    # Check if room exists and has reached the limit (2 users max)
    if room_id in rooms and len(rooms[room_id]) >= 2:
        emit('room_full', {'message': 'Room is full'}, room=sid)
        print(f"Client {sid} attempted to join full room {room_id}")
        return
...
def handle_end_meeting():
    """End meeting: broadcast final notes to all participants in the room and clean up."""
    sid = request.sid
    room_id = sid_to_room.get(sid)
    if not room_id:
        return
    ...
    # Clean up room state
    if room_id in rooms:
        # Remove all members from sid_to_room mapping
        for member_sid in rooms[room_id]:
            sid_to_room.pop(member_sid, None)
        # Remove room
        del rooms[room_id]
    room_state.pop(room_id, None)
    
    print(f"Meeting ended in room {room_id}, room cleaned up")
